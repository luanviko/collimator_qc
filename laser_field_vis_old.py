...
# Equation: var_mm = (a**2 * var_px) + (px**2 * var_a) + var_b + (2 * px * cov_ab)
xo_err_mm = np.sqrt(
    (a_x**2 * xo_err_px**2) + 
    (xo_px**2 * mm_var_a_x) + 
    mm_var_b_x + 
...
""" FIT Y """
m = 5
y_coeff_1, y_cov_1 = np.polyfit(z, yo, 1, w=1.0/yo_err_mm, cov='unscaled')
y_coeff_n, y_cov_n = np.polyfit(z, yo, m, w=1.0/yo_err_mm, cov='unscaled')
y_fit_1   = np.poly1d(y_coeff_1)
y_fit_n   = np.poly1d(y_coeff_n)

""" FIT X """
n = 5
x_coeff_n, x_cov_n = np.polyfit(z, xo, n, w=1.0/xo_err_mm, cov='unscaled')
x_coeff_1, x_cov_1 = np.polyfit(z, xo, 1, w=1.0/xo_err_mm, cov='unscaled')
